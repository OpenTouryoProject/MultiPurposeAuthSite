...
namespace MultiPurposeAuthSite.Controllers
{
    ...
    public class OAuth2EndpointController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetUserClaims()
        {
                ...
                MyHttpContext.Current.Request.Headers[OAuth2AndOIDCConst.HttpHeader_Authorization], out string bearerToken))
            {
                if (Token.CmnAccessToken.VerifyAccessToken(bearerToken, out JObject claims, out ClaimsIdentity identity))
                {
                    ...
                    if (!Token.CmnAccessToken.VerifyCertificateBinding(identity, Request.HttpContext.Connection.ClientCertificate))
                    {
        ...
        [HttpPost]
        public IActionResult DeviceAuthZAuthorize(IFormCollection formData)
        {
            // **資格情報・属性を返すので、キャッシュに残さない**（#218）。
            //   RFC は MUST としていないが、困る性質は /token と同じ。
            this.SetNoStore();
        ...
        [HttpPost]
        public async Task<IActionResult> CibaAuthorizeAsync(IFormCollection formData)
        {
            // **資格情報・属性を返すので、キャッシュに残さない**（#218）。
            //   RFC は MUST としていないが、困る性質は /token と同じ。
            this.SetNoStore();
...
#pragma warning restore 162
        ...
        public IActionResult CibaPushResult(IFormCollection formData)
        {
            ...
            // 証明書に紐づくトークンは、その証明書を提示した要求でしか使えない（RFC 8705 3）
            if (Token.CmnAccessToken.VerifyAccessToken(bearerToken, out JObject claims, out ClaimsIdentity identity)
                && Token.CmnAccessToken.VerifyCertificateBinding(identity, Request.HttpContext.Connection.ClientCertificate))
            {
                // ClientIdの取り出し
                Claim ClientId = identity.Claims.Where(
                    x => x.Type == OAuth2AndOIDCConst.UrnAudienceClaim).FirstOrDefault<Claim>();
        ...
        /// </remarks>
        [HttpPost]
        public IActionResult PushedAuthorizationRequest(IFormCollection formData)
        {
                ...
                out Dictionary<string, string> ret, out err))
            {
                ...
                return new ObjectResult(ret) { StatusCode = 201 };
            }
        ...
        public async Task<ActionResult> RequestObjectUri()
        {
            ...
            if (Token.CmnEndpoints.RegisterRequestObject(body, out Dictionary<string, object> ret))
            {
                ...
                return this.Created("", // 第一引数...
                    JsonConvert.SerializeObject(ret, Newtonsoft.Json.Formatting.None));
            }
        ...
        {
            IHeaderDictionary headers = MyHttpContext.Current.Response.Headers;
        ...
        /// </remarks>
        private IActionResult NGResult(int status, string realm, string error)
        {
            if (status == 401)
            {
                Dictionary<string, string> err = new Dictionary<string, string>();
            ...
            return new ObjectResult("NG") { StatusCode = status };
        }
