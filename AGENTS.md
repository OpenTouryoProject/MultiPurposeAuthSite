...
#### コミット メッセージの文面に、署名行を付けない

コミット メッセージを提示するときは、**`Co-Authored-By` や `Claude-Session` の行を付けない。**

- **コミットするのは人である。** エージェントは文面を渡すだけなので、共著者として署名する位置にいない
- 既存の履歴は `fixed #196` のような簡潔な 1 行主体で、**署名行はその流儀から浮く**
- セッションの URL は**外部から参照できない**。履歴に残しても辿れない

エージェント自身がコミットするよう指示された場合は、その時の指定に従う（上記は**提示する文面**の話）。
